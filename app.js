...
const getFollowingPeopleIdsUser = async username => {
  ...
  const followingPeople = await db.all(peopleIdQuery)
  const arrayId = followingPeople.map(eachUser => eachUser.following_user_id)
  return arrayId
}
...
app.get('/tweets/:tweetId/likes/', authentication, tweetAccessVerification, async (request, response) =>{
    ...
    const usersArray = likesUsers.map((eachUser) => eachUser.username);
